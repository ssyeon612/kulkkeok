...
<head>
	...
	<script type="text/javascript">
			...
			function deleteChk(postNo){
				...
				if(chk){
					location.href="free.do?command=delete&no="+postNo;
				}else{ return false;}
	...
	<script type="text/javascript">
		...
		function showReplyList(){
					var url = "comment.do?command=commentlist&boardCode=FREE&postNo="+${post.postNo};
		...
		function deleteComment(postNo,commentNo){
			...
			if(chk){
				...
				$.ajax({
						...
						data: {"postNo":postNo, "commentNo":commentNo, "boardCode":"FREE"},
						success : function(data){
							if(data=="true"){
								alert("댓글이 삭제되었습니다.");
								showReplyList();
	...
	<script type="text/javascript">
		function update(postNo,commentNo){
			...
			if(comment==""||comment==null){
				...
				var allData = { "postNo": postNo,"commentNo":commentNo, "boardCode": "FREE", "comment":comment };
...
<body>
    ...
    <!--================start writing area =================-->
        <div>
            <form id="viewForm" name="viewForm" action="free.do?command=updateform" method="post">
            	<input type="hidden" name="postNo" value="${post.postNo }">
                <div id="chgposition">
                    <h2 style="text-align: center;">자유게시판</h2><br>
                    <div>
                        ...
                        <c:if test="${ ( user.userId eq post.userId ) }">
                         ...
                         <c:if test="${ ( ( user.userId eq post.userId ) || user.userId eq 'ADMIN' ) }">
                            <button type="button" class="genric-btn primary radius" id="btnDelete" onclick="deleteChk(${post.postNo});">삭제</button>
                        </c:if>
                        <c:if test="${user.userId ne null }">
                            <button type="button" class="genric-btn primary radius" id="btnReply" onclick="location.href='free.do?command=answerform&parentNo='+${post.postNo}">답변</button>                        
                        </c:if>
                        
                            <button type="button" class="genric-btn primary radius" id="btnList" onclick="location.href='free.do?command=list'">목록</button>
                        </div>
                    </div>
                </div>
            </form>
                                ...
                                <tr>
                                    <td>
	                                    <c:choose>
	                                    	...
	                                    	<c:otherwise>
	                                         	<textarea class="form-control" style="width: 1100px; resize: none;" rows="2" cols="30" id="inputComment" name="comment" placeholder="회원만 댓글작성이 가능합니다." readonly="readonly"></textarea>
		                                        <br>
		                                        <div style="text-align: right;">
